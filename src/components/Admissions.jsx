...
export default function Admissions() {
  ...
  const subtitle = db('subtitle') || a.subtitle

  const steps = [1, 2, 3, 4].map((n, i) => ({
    title: db(`step_${n}_title`) || a.steps[i]?.title,
    desc:  db(`step_${n}_desc`)  || a.steps[i]?.desc,
  }))
      ...
      <div className="container-custom relative">
        <div className="text-center mb-14 reveal">
          ...
          <h2 className="font-heading text-4xl md:text-5xl text-white mb-4">
            {db('section_title') || a.title} <em className="not-italic text-crimson-400">{a.titleEm}</em>
          </h2>
