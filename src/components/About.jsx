...
export default function About() {
  ...
  const c = content
  const db = (key) => lang !== 'ar' && c[key]
  const p1 = db('p1') || a.p1
  const p2 = db('p2') || a.p2
  const missionTitle = db('mission_title') || a.missionTitle
  const missionText  = db('mission_text')  || a.missionText
  const values = [
    { title: db('value_1_title') || a.values[0]?.title, desc: db('value_1_desc') || a.values[0]?.desc },
    { title: db('value_2_title') || a.values[1]?.title, desc: db('value_2_desc') || a.values[1]?.desc },
    { title: db('value_3_title') || a.values[2]?.title, desc: db('value_3_desc') || a.values[2]?.desc },
    { title: db('value_4_title') || a.values[3]?.title, desc: db('value_4_desc') || a.values[3]?.desc },
  ]
