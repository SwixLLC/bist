...
export default function Teaching() {
  ...
  const p1 = db('p1') || p.p1
  const p2 = db('p2') || p.p2
  const quote = db('quote') || p.quote
  const quoteAuthor = db('quote_author') || p.quoteAuthor
  const methods = Array.from({ length: 6 }, (_, i) => ({
    title: db(`method_${i + 1}_title`) || p.methods[i]?.title,
    desc:  db(`method_${i + 1}_desc`)  || p.methods[i]?.desc,
  }))
