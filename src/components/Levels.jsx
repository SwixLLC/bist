...
export default function Levels() {
  ...
  const subtitle = db('section_subtitle') || l.subtitle
  const levels = levelConfig.map(({ id }, i) => ({
    name:        db(`${id}_name`)     || l.items[i]?.name,
    ages:        db(`${id}_ages`)     || l.items[i]?.ages,
    description: db(`${id}_desc`)    || l.items[i]?.description,
    features:    db(`${id}_features`)
      ? db(`${id}_features`).split('\n').filter(Boolean)
      : l.items[i]?.features,
  }))
