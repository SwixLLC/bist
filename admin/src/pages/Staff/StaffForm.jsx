...
export default function StaffForm() {
  ...
  useEffect(() => {
    if (!isEdit) return
    supabase.from('staff').select('*').eq('id', id).single().then(({ data, error }) => {
      if (error || !data) { toast(t.common.error + ' — ' + (error?.message || 'Membre introuvable'), 'error'); navigate('/staff'); return }
      setForm({
  ...
  async function handleSubmit(e) {
    ...
    if (!form.role.trim()) return toast('Poste requis', 'error')
    setLoading(true)
    try {
      if (isEdit) {
