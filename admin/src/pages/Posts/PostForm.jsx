...
export default function PostForm() {
  ...
  useEffect(() => {
    ...
    supabase.from('posts').select('*').eq('id', id).single()
      .then(({ data, error }) => {
        if (error || !data) { toast(t.common.error + ' — ' + (error?.message || 'Post introuvable'), 'error'); navigate('/posts'); return }
        setForm({ title: data.title || '', content: data.content || '', excerpt: data.excerpt || '', cover_image: data.cover_image || '', status: data.status || 'draft' })
      })
