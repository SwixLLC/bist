...
export default function EventForm() {
  ...
  const [form, setForm] = useState({ title: '', description: '', start_date: nowLocal(), end_date: '', location: '', status: 'upcoming', cover_image: '' })
  ...
  useEffect(() => {
    ...
    supabase.from('events').select('*').eq('id', id).single().then(({ data, error }) => {
      if (error || !data) { toast(t.common.error + ' — ' + (error?.message || 'Événement introuvable'), 'error'); navigate('/events'); return }
      setForm({
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    if (!form.title.trim()) return toast('Titre de l\'événement requis', 'error')
    if (!form.start_date) return toast('Date de début requise', 'error')
    setLoading(true)
    try {
      if (isEdit) {
